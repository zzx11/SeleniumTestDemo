...
import unittest
import time
from com.selenium.common.HTMLTestRunner_PY3 import HTMLTestRunner
...
if __name__ == "__main__":
    # 测试用例的路径、匹配规则
    case_path = os.path.join(os.getcwd(), "test_suites")
    rule = "equip_instore_test.py"
    all_case = add_case(case_path, rule)  # 1加载用例
    # 生成测试报告的路径
    report_path = os.getcwd() + '/test_report/'
    # report_path = "D:\\Projects\\PycharmProjects\\WebUIFramework\\test_report\\"
    run_case(all_case, report_path)  # 2执行用例
    # 获取最新的测试报告文件
    # report_file = get_report_file(report_path)  # 3获取最新的测试报告
    # # 邮箱配置，发送邮件
    # sender = "859904468@qq.com"
    # psw = "lccdemqhvstabfhj"
    # # 收件人多个时，中间用逗号隔开,如'a@xx.com,b@xx.com'
    # receiver = "772758115@qq.com"
